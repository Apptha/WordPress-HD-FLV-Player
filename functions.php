...
// Get input informations from POST
    ...
    if (!empty($act_filepath)) { 
        ...
        if( strpos($act_filepath,'www.youtube.com') > 0 ){
            $youtube_data = hd_GetSingleYoutubeVideo($imageUrl);
    ...
    if (!empty($act_filepath)) {
        if (strpos($act_filepath, 'youtube') > 0 || strpos($act_filepath, 'youtu.be') > 0) {
            ...
            if ($ydetails) {
                ...
                if(isset($ydetails['thumbnail_url'])){
                $act[3] =$ydetails['items'][0]->snippet->thumbnail_url; 
                }
                $act[4] = $act_filepath;
            } else
                hdflv_render_error(__('Could not retrieve Youtube video information', 'hdflv'));
...
//    echo $act_name = preg_replace("/[^a-zA-Z0-9\/_-\s]/", '', $act_name);
    ...
    if (strlen($act_name) > 30)
    $videoName = substr($act_name, 0, 30) . ' ...';
    render_message('Video file '.'"'.$videoName.'" '.__('Updated Successfully', 'hdflv'));
    return $videoName;
}
...
//    $p_name = addslashes(trim($name));
    ...
    if (!empty($p_name)) {
        ...
        if ($insert_plist != 0) {
            ...
            render_message(__('Category', 'hdflv') . ' "' . $name .'"'. __(' added successfully', 'hdflv')) . get_playlist_for_dbx($media);
        }
    }
...
//    $p_name = preg_replace("/[^a-zA-Z0-9\/_-\s]/", '', $p_name);
    ...
    if (!empty($p_name)) {
        ...
        if ($insert_plist != 0) {
            $pid = $wpdb->insert_id;  // get index_id
            render_message(__('Category', 'hdflv') . ' "' . $play_name .'"'. __(' added successfully', 'hdflv'));
        }
    }
...
//    $p_name = preg_replace("/[^a-zA-Z0-9\/_-\s]/", '', $p_name);
    ...
    if (!empty($p_name)) {
        ...
        render_message(__('Category', 'hdflv') . ' "' . $play_name . '" ' . __('Updated Successfully', 'hdflv'));
    }
    return;
}
...
function hd_GetSingleYoutubeVideo($youtube_media) {
    global $wpdb;
    $settingsData = $wpdb->get_row("SELECT player_values FROM " . $wpdb->prefix . "hdflv_settings");
    $player_values = unserialize($settingsData->player_values);
    if ($youtube_media == '')
        return;
        $youtube_key=$player_values[youtube_api];
